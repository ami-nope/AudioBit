...
public sealed class InstallerEngineTests
{
    ...
    public async Task InstallAsync_ExtractsPayloadAndReplacesExistingFiles()
    {
        ...
        try
        {
            ...
            Assert.True(File.Exists(Path.Combine(installDir, "AudioBit.App.exe")));
            Assert.True(File.Exists(Path.Combine(installDir, "data", "settings.json")));
    ...
    private static void CreatePayloadZip(string payloadZip)
    {
        ...
        var appEntry = archive.CreateEntry("AudioBit.App.exe");
